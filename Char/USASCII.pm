...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Eusascii::uc($char[$i]) ne Char::Eusascii::lc($char[$i]))) {
            $char[$i] = '[' . Char::Eusascii::uc($char[$i]) . Char::Eusascii::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::USASCII/Char::USASCII/JA.pm
...
Definitions in Char/Eusascii.pm.
  ...
  @{Char::Eusascii::eV}             qr{(?:[^\x0C\x0A\x0D])}
