...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Eusascii::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Eusascii::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Eusascii::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Eusascii::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a US-ASCII $string for delimiters that match pattern and splits the US-ASCII
  ...
  pattern, so the delimiters may be of any size and need not be the same US-ASCII
  $string on every match. If the pattern doesn't match at all, Char::Eusascii::split returns
  the original US-ASCII $string as a single substring. If it matches once, you get
  ...
  If US-ASCII $string is omitted, the function splits the $_ US-ASCII string.
...
=item Chop string
  ...
  Chops off the last character of a US-ASCII string contained in the variable (or
  US-ASCII strings in each element of a @list) and returns the character chopped.
  The Char::Eusascii::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Eusascii::index($string,$substr,$position);
  $pos = Char::Eusascii::index($string,$substr);

  Returns the position of the first occurrence of $substr in US-ASCII $string.
  The start, if specified, specifies the $position to start looking in the US-ASCII
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Eusascii::index function returns -1.

=item Reverse index string

  $pos = Char::Eusascii::rindex($string,$substr,$position);
  $pos = Char::Eusascii::rindex($string,$substr);

  Works just like Char::Eusascii::index except that it returns the position of the last
  occurence of $substr in US-ASCII $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the US-ASCII string the function can search.
...
=item Lower case string
  ...
  Returns a lowercase version of US-ASCII string (or $_, if omitted). This is the
...
=item Lower case first character of string
  ...
  Returns a version of US-ASCII string (or $_, if omitted) with the first character
...
=item Upper case string
  ...
  Returns an uppercased version of US-ASCII string (or $_, if string is omitted).
...
=item Make character
  ...
  set. For example, Char::Eusascii::chr(65) is "A" in either ASCII or US-ASCII, not Unicode,
  and Char::Eusascii::chr(0x82a0) is a US-ASCII HIRAGANA LETTER A. For the reverse of
  Char::Eusascii::chr, use Char::USASCII::ord.
