...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Eusascii::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Eusascii::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Eusascii::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Eusascii::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Eusascii::split returns
...
=item Chop string
  ...
  The Eusascii::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Eusascii::index($string,$substr,$position);
  $pos = Eusascii::index($string,$substr);
  ...
  the Eusascii::index function returns -1.

=item Reverse index string

  $pos = Eusascii::rindex($string,$substr,$position);
  $pos = Eusascii::rindex($string,$substr);

  Works just like Eusascii::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Eusascii::chr(65) is "A" in either ASCII or US-ASCII, not Unicode,
  and Eusascii::chr(0x82a0) is a US-ASCII HIRAGANA LETTER A. For the reverse of
  Eusascii::chr, use USASCII::ord.
