...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $USASCII::re_t=%s; while($USASCII::re_t =~ %s){%s local $^W=0; local $USASCII::re_r=%s; %s$USASCII::re_t="$`$USASCII::re_r$'"; pos($USASCII::re_t)=length "$`$USASCII::re_r"; } return $USASCII::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$USASCII::re_r=CORE::eval $USASCII::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $USASCII::re_r=%s; %s"%s$USASCII::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $USASCII::re_n=0; while(%s =~ %s){%s local $^W=0; local $USASCII::re_r=%s; %s%s="$`$USASCII::re_r$'"; pos(%s)=length "$`$USASCII::re_r"; $USASCII::re_n++} return %s$USASCII::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $USASCII::re_r=%s; %s%s="%s$USASCII::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $USASCII::re_r=%s; %s%s="%s$USASCII::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$USASCII::re_r=CORE::eval $USASCII::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
