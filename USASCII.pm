...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Eusascii::function_ord     = 'USASCII::ord'     if /\A ord \z/oxms;
                    $Eusascii::function_ord_    = 'USASCII::ord_'    if /\A ord \z/oxms;
                    $Eusascii::function_reverse = 'USASCII::reverse' if /\A reverse \z/oxms;
                    $Eusascii::function_getc    = 'USASCII::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= USASCII::escape();
...
=head1 Calling 'Eusascii::ignorecase()' (USASCII software provides)

USASCII software applies calling 'Eusascii::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (USASCII software provides)

USASCII software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (USASCII software provide)
...
=head1 Un-Escaping bytes::* Subroutines (USASCII software provides)

USASCII software removes 'bytes::' at head of subroutine name.
